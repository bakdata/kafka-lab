 ...
 */
@Slf4j
public class RangeProcessor<K, V> implements Processor<K, V, Void, Void> {
     ...
     */
    public RangeProcessor(final String storeName, final RangeIndexer<? super K, ? super V> writeRangeIndex) {
    ...
    public void process(final Record<K, V> record) {
        ...
        if (this.store == null) {
            throw new IllegalStateException("The state store was not initialized.");
        }

        if (value == null) {
            throw new KafkaStorageException("The value should not be null. Check you input topic data.");
        }
